...
import com.example.taskmanagerapp.Adapter.ToDoAdapter
...
class RecyclerItemTouchHelper(private val adapter: ToDoAdapter) : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT) {
    ...
    override fun onChildDraw(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
        ...
        icon = if (dX > 0) {
            ContextCompat.getDrawable(adapter.getContext(), R.drawable.ic_baseline_edit)
        } else {
            ContextCompat.getDrawable(adapter.getContext(), R.drawable.ic_baseline_delete)
        }

        background = if (dX > 0) {
            ColorDrawable(ContextCompat.getColor(adapter.getContext(), R.color.colorPrimaryDark))
        } else {
